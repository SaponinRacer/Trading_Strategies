    ...
    for i in range(0, len(strategy)):
        ...
        if strategy[i]=="Buy":
            ...
            if current_open_sells>0:
                trading_history["Closing Price"].iloc[-current_open_sells:] = price_df["Close"].iloc[i]
                trading_history["Closed on"].iloc[-current_open_sells:] = price_df.index[i]
                incoming_balance = 0
                for j in np.linspace(current_open_sells, 1, current_open_sells):
                    j = int(j)
                    trading_history["Profit/Loss"].iloc[-j] = (trading_history["Total Investment"].iloc[-j]
        ...
        if strategy[i]=="Sell":
            ...
            if current_open_buys>0:
                trading_history["Closing Price"].iloc[-current_open_buys:] = price_df["Close"].iloc[i]
                trading_history["Closed on"].iloc[-current_open_buys:] = price_df.index[i]
                incoming_balance = 0
                for j in np.linspace(current_open_buys, 1, current_open_buys):
                    j = int(j)
                    trading_history["Profit/Loss"].iloc[-j] = (trading_history["Closing Price"].iloc[-j]*trading_history["Number of Shares"].iloc[-j]
    ...
    if close_positions==True:
        for k in trading_history[trading_history["Closing Price"].isna()].index:
            ...
            if trading_history["Action"].iloc[k]=="Buy":
                ...
                trading_history["Profit/Loss"].iloc[k] = (trading_history["Total Investment"].iloc[k]
